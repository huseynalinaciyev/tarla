...
// --- Oyun parametrləri ---
const COIN_PER_PRODUCT = 10; // bitki məhsulu satışı
...
// --- Yaddaş ---
let data = {
    coins: 20, stock:0, eggs:0, milk:0, chickens:0, cows:0,
...
function plantCrop(){
    ...
    plot.plantStage='seed'; plot.plantedAt=Date.now(); plot.lastWateredAt=0; plot.harvestReady=false;
    updateUI(); alert(`Bitki əkildi! ${PLANT_COST} coin xərcləndi. 15 dəq ərzində suvarın.`);
}
...
function waterCrop(){
    ...
    updateUI(); alert('Bitki suvarıldı! 30 dəq sonra məhsul yığa bilərsiniz.');
}
...
function harvestCrop(){
    ...
    data.stock++;
    plot.plantStage='empty'; plot.harvestReady=false; plot.plantedAt=0; plot.lastWateredAt=0;
    updateUI(); alert(`Məhsul yığdınız! ${COIN_PER_PRODUCT} coin qazandınız.`);
}
